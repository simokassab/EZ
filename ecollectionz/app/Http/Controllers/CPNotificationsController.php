...
class CPNotificationsController extends Controller
{
    ...
    public function getNotifications(){
        ...
        $id=auth()->user()->id;
        DB::enableQueryLog();
        $unread = DB::table('notifications_cp')->where('isread', '=', '0')
            ->where('to_', $id)
    ...
    public function marAsRead($id){
        DB::enableQueryLog();
        DB::table('notifications_cp')->where('id', $id)->update(['isread' => 1]);
        $not_url = DB::table('notifications_cp')->where('id', '=', $id)->get();
        //dd(DB::getQueryLog());
        echo $not_url[0]->href;
    }
