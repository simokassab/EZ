...
class BrokersController extends Controller
{
    ...
    public function index()
    {
       ...
       // dd(Auth::user());
        ...
        INNER JOIN brokers ON B.broker_id =brokers.bk_id where B.broker_id="'.$id.'" and B.status = "P"');
        //
       ...
       //
        ...
        status, max(created_at) as created_at from policies group by client_name, phone, draft_no ) A 
        INNER JOIN policies B USING (client_name, phone, draft_no,created_at)
        INNER JOIN brokers ON B.broker_id =brokers.bk_id where B.broker_id="'.$id.'"  GROUP BY B.status');
        return view('brokers.dashboard')->with('paid_policies', $paid_policies)
    ...
    public function getClients(){
        ...
        return view ('brokers.clients')->with('clients', $clients)->with('gpa', $gpa);
    }
    ...
    public function getPoliciesByClient($phone){
        ...
        where B.broker_id='.$id.' and B.phone='.$phone);
        //dd(DB::getQueryLog());
        return view('brokers.policies')->with('policies', $policies);
    ...
    public function getAllPolicies (){
      ...
      //  $search=DB::table('policies')->where('cust_id', '1234567890');
        $policies = DB::select('
        SELECT corporates.name as CP_NAME, 
        B.* FROM ( select client_name, phone, draft_no, status, max(created_at) as created_at
        from policies group by client_name, phone, draft_no ) A
        INNER JOIN policies B USING (client_name, phone, draft_no,created_at)
        INNER JOIN corporates ON B.cust_id =corporates.id

        where B.broker_id='.$id);
        //  dd(DB::getQueryLog());
        return view('brokers.allpolicies');
    }
    ...
    public function getHistory($id){
        ...
        $history = DB::table('policies')
            ...
            ->where('policies.broker_id', $bk_id)
            ->select('policies.*')
            ->get();
        return view('brokers.history')->with('history', $history);
    }
    ...
    public function searchPolicies (){
        ...
        return view('brokers.searchpolicies')->with('search', $search);
    }
    ...
    public function getSearchPolicies (Request $request){
       ...
       // dd(DB::getQueryLog());
        return view('brokers.searchpolicies')->with('search', $search)->with('request', $request);
    }
    ...
    public function getComments($p_id){
        ...
        $draft=str_replace('!', '\\', $draft);
        $phone=$str[2];
        $corp = DB::table('brokers')->where('bk_id', $bk_id)->get();
        $policies = DB::table('policies')
        ->where('broker_id', $bk_id)
        ->where('draft_no', $draft)
        ->where('phone', $phone)
            ->get();
        ...
        ->join('brokers', 'corporates.id', 'brokers.CP_ID_FK')
                ->where('bk_id', $bk_id)
                ->where('draft_no', $draft)
        ...
        $replies = DB::table('comments')->leftJoin('replies', 'comments.id', '=', 'replies.comment_id')
            ->where('corporate_id', $corp[0]->CP_ID_FK)
            ->where('draft_no', $draft)
            ->where('comments.phone', $phone)
            ->get();
       // dd(DB::getQueryLog());
        ...
        return view('brokers.comments')->with('comments', $comments)->with('replies', $replies)->with('policies', $policies);
    }
    ...
    public function getPoliciesByStatus($status)
    {
        ...
        if($status=='UN'){
            $policies = DB::select('
                ...
                INNER JOIN policies B USING (client_name, phone, draft_no,created_at)
                INNER JOIN corporates ON B.cust_id =corporates.id
                where B.broker_id='.$id.' and B.status=""');
        }
        else {
            $policies = DB::select('
                SELECT corporates.name as CP_NAME,
                ...
                INNER JOIN policies B USING (client_name, phone, draft_no,created_at)
                INNER JOIN corporates ON B.cust_id =corporates.id
                where B.broker_id='.$id.' and B.status="'.$status.'"');
        }

         //dd(DB::getQueryLog());
        //$corp = DB::table('corporates')->get();
        return view('brokers.getstatus')->with('policies',$policies)->with('status', $status);
    }
    ...
    public function getProfile(){
        $id= Auth()->user()->id;
        $profile =DB::table('brokers')
            ->where('id', $id)
            ->get();
        return view('brokers.profile')->with('profile',$profile);
    }
    ...
    public function updateProfile(Request $request){
        ...
        return redirect('./brokers/profile')->with('success', 'Your profile has been updated !');
    }
